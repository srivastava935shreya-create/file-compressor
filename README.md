...
Metadata Size	4 Bytes	Big-Endian Integer	Length (
N
N) of the serialized JSON frequency table.
Padding Length	1 Byte	Big-Endian Integer	Number of padding bits (0–7) appended to align the final byte.
Frequency Table	
N
N Bytes	UTF-8 JSON String	Serialized byte-frequency dictionary used to reconstruct the tree.
...
Open Command Prompt (cmd) or PowerShell and navigate to the folder:
cmd
...
Let 
N
N be the number of characters in the input text, and 
K
K be the number of unique characters (max 256 for byte-level).
...
O(KlogK) (Inserting and popping from the Priority Queue takes 
O
(
log
⁡
...
O(logK) per operation).
Code Generation: 
O
(
K
)
O(K) (Traversal of the binary tree).
...
O(K) (Very memory-efficient!).
👥 Authors
Shreya Srivastava - GitHub Profile
